...
def get_gnrplan(nNodes, disksPerNode, sparePdisks, pdiskSize,
    ...
    debug_names = ['pdiskEndReserved', 'partitionSize', 'partitionsPerPdisk',
                   ...
                   'maxLogGroups', 'logHomeSize', 'adjustedLoghomeSize',
    ...
    fudge = float(logHomeSize) / float(nNodes)
    adjustedLoghomeSize = int(logHomeSize) + int(math.ceil(fudge))
...
def get_input():
    ...
    parser.add_argument('-x', '--include-loghome-vdisks', action='store_false',
                        default=True,
    ...
    if not args.include_loghome_vdisks:
        print('ERROR:  Include loghome vdisks in sizing estimate ' +
              'does not work yet')
        sys.exit(1)
...
if __name__ == '__main__':
    ...
    if not jsonfmt:
        ...
        print('Include loghome vdisks? {:>40}'.format(repr(loghomes)))
